...
class Simulation:
    def __init__(self,starting_infected,infected_growth_rate,awareness,
                 awareness_growth_rate,mortality_day_rate,recovery_day_rate,
                 recovery_multiplier,containment_effectiveness,cycles):
    ...
    def change(self):
        new_recovered = self.infected[-1] * (self.recovery_day_rate + (self.recovery_day_rate*self.awareness*self.recovery_multiplier))
        new_dead = self.infected[-1] * self.mortality_day_rate
        # containment effectiveness [0,1], 0: not effective, 1: totally effective
        new_infected = ((self.infected[-1]-(self.known[-1]*self.containment_effectiveness))*self.infected_growth_rate)
    ...
    def awareness_growth(self):
        if self.awareness + (self.awareness*self.awareness_growth_rate) > 1:
            ...
            self.awareness = self.awareness + (self.awareness*(self.awareness_growth_rate))
    ...
    def export(self):
        return pd.DataFrame({'INFECTED':self.infected,'KNOWN':self.known,'RECOVERED':self.recovered,'DEAD':self.dead}).astype(int)
    ...
    def plot(self):
        ...
        for i in range(4):
            fig.add_trace(graph_objects.Scatter(x=list(range(len(data[i]))), y=data[i],
    ...
    def bar(self):
        ...
        df = infected.append(known).append(recovered).append(dead)

        df.reset_index(inplace=True,drop=True)

        fig = px.bar(df, x="METRIC", y="COUNT", color="METRIC",
                     animation_frame="TIME", range_y=[0,df['COUNT'].max() + round(df['COUNT'].max()*0.1)])
